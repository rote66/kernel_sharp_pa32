...
#define SHUB_DIAG_BARO   0x04  /* SHMDS_HUB_0120_01 add */
...
static int initial_baro_flg = 0; /* SHMDS_HUB_0120_01 add */
...
static int shub_diag_baroSensorReadSendCmd(uint8_t addr, uint8_t* resData)
{
    ...
    uint16_t errRes;
    uint8_t prm[12];
    uint8_t Data[2];  /* SHMDS_HUB_0120_04 add */
    ...
    shub_direct_get_error(&errRes);
    if((ret == 0) && (errRes == 0)) {
/* SHMDS_HUB_0120_04 mod S */
//        shub_direct_get_result(resData);
        shub_direct_get_result(&Data[0]);
        resData[0] = Data[1];
/* SHMDS_HUB_0120_04 mod E */
    }else{
        printk("[shub]%s err.addr=%02x ret=%d errRes=%d\n", __func__, addr, ret, errRes);
        return -1;
    }
...
static int shub_diag_acc_mode_change(int mode)
{
    ...
    if(mode){
        ...
        setData = 0x7C; /* SHMDS_HUB_2101_01 mod (0x78->0x7C) */
    }
    else{
        /* power down mode */
        setData = 0x00;
...
static int shub_diag_mag_initialize_collect(void)
{
    ...
    if(ret != 0) {
        return MAG_ERROR_INITIALIZE_ERROR;  /* SHMDS_HUB_0323_01 mod (-1 -> MAG_ERROR_INITIALIZE_ERROR) */
    }
    /* cal data read setting */
    ret = shub_diag_mag_read_cal();
    if(ret != 0) {
        return MAG_ERROR_CAL_VER;  /* SHMDS_HUB_0323_01 mod (-1 -> MAG_ERROR_CAL_VER) */
    }
    /* mag initial2 setting */
    ret = shub_diag_mag_initialize_2();
    if(ret != 0) {
        return MAG_ERROR_INITIALIZE2_ERROR;  /* SHMDS_HUB_0323_01 mod (-1 -> MAG_ERROR_INITIALIZE2_ERROR) */
    }
    return 0;
}
    ...
    for(mes_cnt = 0; mes_cnt <= YAS537_RCOIL_RETRY_MAX; mes_cnt++) {
        if(rcoil_setting) {
            ...
            if(ret != 0) {
                ...
                return MAG_ERROR_COIL_SETTING;    /* SHMDS_HUB_0323_01 mod (-1 -> MAG_ERROR_COIL_SETTING) */
            }
            rcoil_setting = 0;
        }
        /* mag measure start */
        ...
        if(ret != 0) {
            return MAG_ERROR_MEASURE;    /* SHMDS_HUB_0323_01 mod (-1 -> MAG_ERROR_MEASURE) */
        }
...
//        temperature = (uint16_t)((mesData[0+1] << 8) | mesData[1+1]);
    ...
    return MAG_ERROR_RETRY_OVER;    /* SHMDS_HUB_0323_01 mod (-1 -> MAG_ERROR_RETRY_OVER) */
}
        ...
        case SHUB_DIAG_SENSOR_INIT:
            {
                ...
                initial_acc_flg = 0;
                initial_mag_flg = 0;
                initial_baro_flg = 0;       /* SHMDS_HUB_0120_01 add */
                ret = copy_to_user(argp, &res, sizeof(struct IoctlDiagRes));
                if (ret) {
        ...
        case SHUB_DIAG_MES_BARO_HWTEST:
            {
                ...
                if(!initial_baro_flg){
                    res.rtn = shub_diag_baro_initialize();
                    if(res.rtn == 0){
                        res.rtn = shub_diag_baroSensorWriteSendCmd(LPS22HB_CTRL_REG1, 0x22);  /* SHMDS_HUB_0120_05 add */
                        initial_baro_flg = 1;
                    }
                }
                if(res.rtn == 0){
