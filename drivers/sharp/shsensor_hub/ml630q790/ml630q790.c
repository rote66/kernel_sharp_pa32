...
#ifdef CONFIG_ACC_U2DH /* SHMDS_HUB_0114_02 SHMDS_HUB_2901_01 */
...
extern int32_t init_i2c(void);    /* SHMDS_HUB_0114_01 add */
...
static int32_t shub_activate_exec(int32_t arg_iSensType, int32_t arg_iEnable, int mode); /* SHMDS_HUB_0337_01 mod */
...
int shub_api_get_face_down_info(struct shub_face_acc_info *info)
{
    ...
    if(((iCurrentSensorEnable & (SHUB_ACTIVE_ACC | SHUB_ACTIVE_SHEX_ACC)) == 0) || s_is_suspend || shub_access_flg){ /* SHMDS_HUB_1301_02 add */
        info->nJudge = 0;
        info->nStat = 0;
        info->nX = 0;
        info->nY = 0;
        info->nZ = 0;
        DBG(DBG_LV_DATA, "get_face_info( Judge=0, stat=0(0x%x), XYZ=0, is_suspend=%d, flg=0x%x )\n", iCurrentSensorEnable, s_is_suspend, shub_access_flg); /* SHMDS_HUB_0304_02 mod */
        return SHUB_RC_OK;
    }
...
int shub_api_get_face_check_info(struct shub_face_acc_info *info)
{
    ...
    if(((iCurrentSensorEnable & (SHUB_ACTIVE_ACC | SHUB_ACTIVE_SHEX_ACC)) == 0) || s_is_suspend || shub_access_flg){ /* SHMDS_HUB_0304_02 mod */
        info->nJudge = 0;
        info->nStat = 0;
        info->nX = 0;
        info->nY = 0;
        info->nZ = 0;
        DBG(DBG_LV_DATA, "get_face_check( Judge=0, stat=0(0x%x), XYZ=0, is_suspend=%d, flg=0x%x )\n", iCurrentSensorEnable, s_is_suspend, shub_access_flg); /* SHMDS_HUB_0304_02 mod */
        return SHUB_RC_OK;
    }
...
{
/* SHMDS_HUB_0120_01 SHMDS_HUB_0122_01 SHMDS_HUB_0704_01 S */
//#ifdef CONFIG_BARO_SENSOR		/* SHMDS_HUB_2603_01 del */
    printk("[shub][dbg] irq=%lld,cmd=%lld,a=%lld,b=%lld,m=%lld,g=%lld,f=%lld,c=%lld,o=%lld,no=%lld,en=%08x(%08x)\n",
...
//#endif
            shub_dbg_cnt_gyro, shub_dbg_cnt_fusion, shub_dbg_cnt_cust, shub_dbg_cnt_other, shub_dbg_cnt_none,
            atomic_read(&g_CurrentSensorEnable), atomic_read(&g_CurrentLoggingSensorEnable));
/* SHMDS_HUB_0120_01 SHMDS_HUB_0122_01 SHMDS_HUB_0704_01 E */
}
...
static int32_t hostif_write_exe(uint8_t adr, const uint8_t *data, uint16_t size)
{
    ...
    return hostif_write_proc(adr, data, size);
}
...
static int32_t hostif_read_exe(uint8_t adr, uint8_t *data, uint16_t size)
{
    ...
    return hostif_read_proc(adr, data, size);
}
/* SHMDS_HUB_0705_01 add E */
...
#ifdef CONFIG_ACC_U2DH /* SHMDS_HUB_0326_01 SHMDS_HUB_2901_01 */
    /***** Set Freerun *****/
...
static int32_t shub_set_param_exec(int32_t type , int32_t *param)
{
    ...
    if(type == APP_PEDOMETER){
        ...
        ret = shub_activate_exec(SHUB_ACTIVE_EXT_PEDOM, param[0], 0); /* SHMDS_HUB_0337_01 mod */
        if(ret != SHUB_RC_OK) {
            return SHUB_RC_ERR;
        }
...
/* SHMDS_HUB_0204_02 add E */
        ...
        ret = shub_hostcmd(&cmd, &res, EXE_HOST_ALL, 14); // SHMDS_HUB_0204_23 add
        if((SHUB_RC_OK != ret) || (0 != res.err.u16)) {
...
/* SHMDS_HUB_2603_03 add E */
        ...
        ret = shub_activate_exec(SHUB_ACTIVE_GDEC, param[0], 0); /* SHMDS_HUB_0337_01 mod */
        if(ret != SHUB_RC_OK) {
            return SHUB_RC_ERR;
        }
...
/* SHMDS_HUB_0332_01 add E */
        ...
        ret = shub_activate_exec(SHUB_ACTIVE_MOTIONDEC, param[0], 0); /* SHMDS_HUB_0337_01 mod */
        if(ret != SHUB_RC_OK) {
            return SHUB_RC_ERR;
        }
...
/* SHMDS_HUB_2701_01 add E */
        ...
        ret = shub_activate_exec(SHUB_ACTIVE_PICKUP, param[0], 0); /* SHMDS_HUB_0337_01 mod */
        if(ret != SHUB_RC_OK) {
            return SHUB_RC_ERR;
        }
        cmd.cmd.u16 = HC_PICKUP_SET_ENABLE;
...
/* SHMDS_HUB_2301_01 add S */
        ...
        ret = shub_activate_exec(SHUB_ACTIVE_TWIST, param[0], 0); /* SHMDS_HUB_0337_01 mod */
        if(ret != SHUB_RC_OK) {
            return SHUB_RC_ERR;
        }
...
/* SHMDS_HUB_0342_01 add S */
    if (arg_iEnable == POWER_ENABLE) {
        logging_flush_exec();
    }
/* SHMDS_HUB_0342_01 add E */
...
/* SHMDS_HUB_0120_01  SHMDS_HUB_0122_01 add E */
    ...
    ret = shub_activate_exec(0, 0, 0);//update sensor measure cycle /* SHMDS_HUB_0337_01 mod */
    if(SHUB_RC_OK != ret){
...
//     if(boot){
//         shub_fw_reset(SHUB_FW_RESET_BOOT);
//     }else{
//         ret = shub_fw_update();
//         if(ret != SHUB_RC_OK) {
//             goto ERROR;
...
int32_t shub_activate(int32_t arg_iSensType, int32_t arg_iEnable)
{
    ...
    ret = shub_activate_exec(arg_iSensType, arg_iEnable, 0); /* SHMDS_HUB_0337_01 mod */
    if(SHUB_RC_OK != ret) {
        goto ERROR;
    }
...
#ifdef SHUB_SUSPEND
    shub_activate_exec(0, 0, 1);//update /* SHMDS_HUB_0337_01 mod */
#endif
    resumeBatchingProc();
