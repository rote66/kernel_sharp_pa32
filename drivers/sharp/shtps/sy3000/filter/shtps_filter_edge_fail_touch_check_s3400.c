...
/* -------------------------------------------------------------------------- */
static u8 shtps_filter_edge_fail_touch_area_check(struct shtps_rmi_spi *ts, unsigned short x, unsigned short y)
{
	u8 area = 0;
	...
	if(y >= SHTPS_EDGE_FAIL_TOUCH_REJECT_AREA_THRESH_Y){
		...
		else if(x >= (SHTPS_GET_PANEL_SIZE_X(ts) - 1 - SHTPS_EDGE_FAIL_TOUCH_REJECT_AREA_THRESH_X)){
			area = 2;
		}
	}
...
/* -------------------------------------------------------------------------- */
static u8 shtps_filter_edge_fail_touch_inhibit_cancel_area_check(struct shtps_rmi_spi *ts, unsigned short x, unsigned short y)
{
	u8 is_cancel = 0;

	SHTPS_LOG_FUNC_CALL();

	if( (x > SHTPS_EDGE_FAIL_TOUCH_REJECT_CANCEL_AREA_THRESH_X) &&
		(x < (SHTPS_GET_PANEL_SIZE_X(ts) - 1 - SHTPS_EDGE_FAIL_TOUCH_REJECT_CANCEL_AREA_THRESH_X)) )
	{
		is_cancel = 1;
	}
...
/* -------------------------------------------------------------------------- */
static int shtps_filter_edge_fail_touch_middle_area_check(struct shtps_rmi_spi *ts, unsigned short x, unsigned short y)
{
	...
	if( SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_TOP_ENABLE == 1){
		if(y <= SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_THRESH_Y){
			if( (x <= SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_THRESH_X) ||
				(x >= (SHTPS_GET_PANEL_SIZE_X(ts) - 1 - SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_THRESH_X)) )
			{
	...
	if( SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_SIDE_ENABLE == 1){
		if(y <= SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_SIDE_AREA_THRESH_Y){
			if( (x <= SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_SIDE_AREA_THRESH_X) ||
				(x >= (SHTPS_GET_PANEL_SIZE_X(ts) - 1 - SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_SIDE_AREA_THRESH_X)) )
			{
	...
	if( SHTPS_EDGE_FAIL_TOUCH_REJECT_BOTTOM_AREA_SIDE_ENABLE == 1){
		if(y >= SHTPS_EDGE_FAIL_TOUCH_REJECT_BOTTOM_SIDE_AREA_THRESH_Y){
			if( (x <= SHTPS_EDGE_FAIL_TOUCH_REJECT_BOTTOM_SIDE_AREA_THRESH_X) ||
				(x >= (SHTPS_GET_PANEL_SIZE_X(ts) - 1 - SHTPS_EDGE_FAIL_TOUCH_REJECT_BOTTOM_SIDE_AREA_THRESH_X)) )
			{
	...
	for(i = 0; i < fingerMax; i++){
		...
		if( (ts->edge_fail_touch_rej_p->edge_fail_touch_top_inhibit_id & (1 << i)) != 0 ){
			if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER)
			{
				...
				now_area = shtps_filter_edge_fail_touch_middle_area_check(ts, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y);
				td_area = shtps_filter_edge_fail_touch_middle_area_check(ts, ts->edge_fail_touch_rej_p->edge_fail_touch_top_td_info.fingers[i].x,
		...
		if( (ts->edge_fail_touch_rej_p->edge_fail_touch_top_inhibit_id & (1 << i)) == 0 ){
				...
				(ts->report_info.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH) )
			{
				...
				area = shtps_filter_edge_fail_touch_middle_area_check(ts, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y);
...
//					if(ts->fw_report_info.fingers[i].z <= SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_THRESH_Z)
					...
					if(SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_RELATION_ZW_RATIO > 0){
						...
						if((w * 100) > ((ts->fw_report_info.fingers[i].z * SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_RELATION_ZW_RATIO) - SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_RELATION_ZW_DIFF) ){
							ts->edge_fail_touch_rej_p->edge_fail_touch_top_inhibit_id |= (1 << i);
							SHTPS_LOG_DBG_EDGE_FAIL_TOUCH_PRINT("[top]inhibit [%d]<x=%d, y=%d, wx=%d, wy=%d, z=%d> relation z/w\n",
																	i, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y,
																	ts->fw_report_info.fingers[i].wx, ts->fw_report_info.fingers[i].wy, ts->fw_report_info.fingers[i].z);
						}
					}

					if(SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_RELATION_ZWY_RATIO > 0){
						u8 w = ts->fw_report_info.fingers[i].wy;

						if((w * 100) > ((ts->fw_report_info.fingers[i].z * SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_RELATION_ZWY_RATIO) - SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_RELATION_ZWY_DIFF) ){
							ts->edge_fail_touch_rej_p->edge_fail_touch_top_inhibit_id |= (1 << i);
							SHTPS_LOG_DBG_EDGE_FAIL_TOUCH_PRINT("[top]inhibit [%d]<x=%d, y=%d, wx=%d, wy=%d, z=%d> relation z/wy\n",
																	i, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y,
																	ts->fw_report_info.fingers[i].wx, ts->fw_report_info.fingers[i].wy, ts->fw_report_info.fingers[i].z);
						}
					}
		...
		if( (ts->edge_fail_touch_rej_p->edge_fail_touch_inhibit_id & (1 << i)) != 0 ){
			...
			if( (ts->edge_fail_touch_rej_p->edge_fail_touch_bottom_inhibit_id & (1 << i)) != 0 ){
				if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER)
				{
					...
					now_area = shtps_filter_edge_fail_touch_middle_area_check(ts, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y);
					td_area = shtps_filter_edge_fail_touch_middle_area_check(ts, ts->edge_fail_touch_rej_p->edge_fail_touch_bottom_td_info.fingers[i].x,
					...
					(ts->report_info.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH) )
				{
						...
						((area & SHTPS_EDGE_FAIL_TOUCH_REJECT_MIDDLE_AREA_BOTTOM_SIDE) != 0) )
					{
						if(SHTPS_EDGE_FAIL_TOUCH_REJECT_BOTTOM_AREA_RELATION_ZW_RATIO > 0){
							...
							if((w * 100) > ((ts->fw_report_info.fingers[i].z * SHTPS_EDGE_FAIL_TOUCH_REJECT_BOTTOM_AREA_RELATION_ZW_RATIO) - SHTPS_EDGE_FAIL_TOUCH_REJECT_BOTTOM_AREA_RELATION_ZW_DIFF)){
								ts->edge_fail_touch_rej_p->edge_fail_touch_bottom_inhibit_id |= (1 << i);
								SHTPS_LOG_DBG_EDGE_FAIL_TOUCH_PRINT("[bottom]inhibit [%d]<x=%d, y=%d, wx=%d, wy=%d, z=%d> relation z/w\n",
																		i, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y,
																		ts->fw_report_info.fingers[i].wx, ts->fw_report_info.fingers[i].wy, ts->fw_report_info.fingers[i].z);
							}
						}

						if(SHTPS_EDGE_FAIL_TOUCH_REJECT_BOTTOM_AREA_RELATION_ZWY_RATIO > 0){
							u8 w = ts->fw_report_info.fingers[i].wy;

							if((w * 100) > ((ts->fw_report_info.fingers[i].z * SHTPS_EDGE_FAIL_TOUCH_REJECT_BOTTOM_AREA_RELATION_ZWY_RATIO) - SHTPS_EDGE_FAIL_TOUCH_REJECT_BOTTOM_AREA_RELATION_ZWY_DIFF)){
								ts->edge_fail_touch_rej_p->edge_fail_touch_bottom_inhibit_id |= (1 << i);
								SHTPS_LOG_DBG_EDGE_FAIL_TOUCH_PRINT("[bottom]inhibit [%d]<x=%d, y=%d, wx=%d, wy=%d, z=%d> relation z/wy\n",
																		i, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y,
																		ts->fw_report_info.fingers[i].wx, ts->fw_report_info.fingers[i].wy, ts->fw_report_info.fingers[i].z);
							}
						}
...
/* -------------------------------------------------------------------------- */
static u8 shtps_filter_edge_fail_touch_side_area_check(struct shtps_rmi_spi *ts, unsigned short x, unsigned short y)
{
	u8 is_side_area = 0;

	if( (x <= SHTPS_EDGE_FAIL_TOUCH_REJECT_SIDE_AREA_THRESH_X) ||
		(x >= (SHTPS_GET_PANEL_SIZE_X(ts) - 1 - SHTPS_EDGE_FAIL_TOUCH_REJECT_SIDE_AREA_THRESH_X)) )
	{
		is_side_area = 1;
	}
...
static void shtps_filter_edge_fail_touch_side_check(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
	...
	for(i = 0; i < fingerMax; i++){
		...
		if( (ts->edge_fail_touch_rej_p->edge_fail_touch_inhibit_id & (1 << i)) != 0 ){
			...
			if( (ts->edge_fail_touch_rej_p->edge_fail_touch_side_inhibit_id & (1 << i)) != 0 ){
				if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER){
					if( shtps_filter_edge_fail_touch_side_area_check(ts, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y) == 0 )
					{
						struct shtps_touch_info rep_info;
						u8 event;
			...
			if( (ts->edge_fail_touch_rej_p->edge_fail_touch_side_inhibit_id & (1 << i)) == 0 ){
					...
					(ts->report_info.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH) )
				{
					if( (shtps_filter_edge_fail_touch_side_area_check(ts, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y) != 0) )
					{
						u8 w = (ts->fw_report_info.fingers[i].wx > ts->fw_report_info.fingers[i].wy) ?
								ts->fw_report_info.fingers[i].wx : ts->fw_report_info.fingers[i].wy;
						u8 wy = (ts->fw_report_info.fingers[i].wy == 0) ? 1 : ts->fw_report_info.fingers[i].wy;
						...
						if(SHTPS_EDGE_FAIL_TOUCH_REJECT_SIDE_AREA_THRESH_ZWY_RATIO > 0){
							if( ((ts->fw_report_info.fingers[i].z * 100) / wy) <= SHTPS_EDGE_FAIL_TOUCH_REJECT_SIDE_AREA_THRESH_ZWY_RATIO ){
								ts->edge_fail_touch_rej_p->edge_fail_touch_side_inhibit_id |= (1 << i);
								SHTPS_LOG_DBG_EDGE_FAIL_TOUCH_PRINT("[side]inhibit by z/wy thresh (id : %d)\n", i);
							}
						}
						...
						if(SHTPS_EDGE_FAIL_TOUCH_REJECT_SIDE_AREA_THRESH_WY > 0){
							if(wy >= SHTPS_EDGE_FAIL_TOUCH_REJECT_SIDE_AREA_THRESH_WY){
								ts->edge_fail_touch_rej_p->edge_fail_touch_side_inhibit_id |= (1 << i);
								SHTPS_LOG_DBG_EDGE_FAIL_TOUCH_PRINT("[side]inhibit by wy thresh (id : %d)\n", i);
							}
						}
...
void shtps_filter_edge_fail_touch_check(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
	...
	for(i = 0; i < fingerMax; i++){
		...
		if( (ts->edge_fail_touch_rej_p->edge_fail_touch_inhibit_id & (1 << i)) != 0 ){
			if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER){
				if( shtps_filter_edge_fail_touch_inhibit_cancel_area_check(ts, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y) != 0 ){
					ts->edge_fail_touch_rej_p->edge_fail_touch_inhibit_id &= ~(1 << i);
	...
	if(ts->edge_fail_touch_rej_p->edge_fail_touch_decide_mt == 0){
		...
		else if(ts->edge_fail_touch_rej_p->edge_fail_touch_td_cnt == 2)
		{
			...
			td_area_1st = shtps_filter_edge_fail_touch_area_check(ts, ts->edge_fail_touch_rej_p->edge_fail_touch_td_info[0].x, ts->edge_fail_touch_rej_p->edge_fail_touch_td_info[0].y);
			td_area_2nd = shtps_filter_edge_fail_touch_area_check(ts, ts->edge_fail_touch_rej_p->edge_fail_touch_td_info[1].x, ts->edge_fail_touch_rej_p->edge_fail_touch_td_info[1].y);
