...
/* -------------------------------------------------------------------------- */
	...
	defined(SHTPS_DEF_GLOVE_DETECT_FAIL_RESOLVE_ENABLE)
struct shtps_filter_correct_info{
	...
	/*	struct shtps_touch_info		hover_reject_pen_chatt_pending;	*/	//AAAAA
		...
		struct delayed_work			glove_touch_up_delayed_work;
		u8							glove_touch_up_delayed_finger;
		u16							glove_detect_fail_resolve_td_x[SHTPS_FINGER_MAX];
		u16							glove_detect_fail_resolve_td_y[SHTPS_FINGER_MAX];
		u8							glove_touch_up_delay_check_state[SHTPS_FINGER_MAX];
	...
	struct shtps_rmi_spi *ts_p;
			...
			SHTPS_DEF_GLOVE_DETECT_FAIL_RESOLVE_ENABLE */
...
/* -------------------------------------------------------------------------- */
#if defined( SHTPS_HOVER_REJECT_ENABLE )
static u8 shtps_hover_reject_area_check(struct shtps_rmi_spi *ts, int x, int y)
{
	u8 is_hover_reject_area = 0;

	if( (x <= SHTPS_HOVER_REJECT_EDGE_AREA) ||
		(x >= (SHTPS_GET_PANEL_SIZE_X(ts) - 1 - SHTPS_HOVER_REJECT_EDGE_AREA)) ||
		(y <= SHTPS_HOVER_REJECT_EDGE_AREA) ||
		(y >= (SHTPS_GET_PANEL_SIZE_Y(ts) - 1 - SHTPS_HOVER_REJECT_EDGE_AREA)) )
	{
...
void shtps_filter_ignore_touch_info_check(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
	...
	if(SHTPS_HOST_HOVER_DETECT_REJECTION_ENABLE != 0)
	{
		...
		for(i = 0; i < fingerMax; i++){
			...
			{
				ts->filter_correct_p->hover_reject_pen_area[i] = shtps_hover_reject_area_check(ts, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y);
			}
			...
			if(ts->filter_correct_p->hover_reject_pen_td_move_check_state[i] == 1){
				if(info->fingers[i].state == SHTPS_TOUCH_STATE_PEN)
				{
					...
					else if(ts->filter_correct_p->hover_reject_zero_info.fingers[i].x >= (SHTPS_GET_PANEL_SIZE_X(ts) - 1 - SHTPS_HOVER_REJECT_EDGE_AREA)){
					...
					else if(ts->filter_correct_p->hover_reject_zero_info.fingers[i].y >= (SHTPS_GET_PANEL_SIZE_Y(ts) - 1 - SHTPS_HOVER_REJECT_EDGE_AREA)){
			...
			if((ts->filter_correct_p->is_hover_finger & (1 << i)) == 0){
				if(ts->fw_report_info_store.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH){
					if(info->fingers[i].state == SHTPS_TOUCH_STATE_PEN)
					{
						...
						if( shtps_hover_reject_area_check(ts, ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y) != 0 ){
		...
		for(i = 0; i < fingerMax; i++){
			if(info->fingers[i].state == SHTPS_TOUCH_STATE_HOVER){
				...
				if( y <= SHTPS_EDGE_HOVER_FAIL_RANGE_Y ||
					y >= ((SHTPS_GET_PANEL_SIZE_Y(ts) - 1) - SHTPS_EDGE_HOVER_FAIL_RANGE_Y) ||
					x <= SHTPS_EDGE_HOVER_FAIL_RANGE_X ||
					x >= ((SHTPS_GET_PANEL_SIZE_X(ts) - 1) - SHTPS_EDGE_HOVER_FAIL_RANGE_X) )
				{
					...
					if(!SHTPS_HOVER_EDGE_LOST_RESOLV_DISABLE){
						if(ts->report_info.fingers[i].state == SHTPS_TOUCH_STATE_HOVER){
							...
							}else if(y >= ((SHTPS_GET_PANEL_SIZE_Y(ts) - 1) - SHTPS_EDGE_HOVER_FAIL_RANGE_Y)){
								info->fingers[i].y = ((SHTPS_GET_PANEL_SIZE_Y(ts) - 1) - SHTPS_EDGE_HOVER_FAIL_RANGE_Y) - 1;
							}
							
							if(x <= SHTPS_EDGE_HOVER_FAIL_RANGE_X){
								info->fingers[i].x = SHTPS_EDGE_HOVER_FAIL_RANGE_X + 1;
							}else if(x >= ((SHTPS_GET_PANEL_SIZE_X(ts) - 1) - SHTPS_EDGE_HOVER_FAIL_RANGE_X)){
								info->fingers[i].x = ((SHTPS_GET_PANEL_SIZE_X(ts) - 1) - SHTPS_EDGE_HOVER_FAIL_RANGE_X) - 1;
							}
							continue;
						}
					}
			...
			if( (1 < SHTPS_HOVER_HIST_COUNT_MAX) && (SHTPS_HOVER_HIST_COUNT_MAX < SHTPS_HOVER_HIST_MAX) ){
				if(ts->filter_correct_p->hover_hist_count >= SHTPS_HOVER_HIST_COUNT_MAX){
					...
					if(hover_report_info_calc_type == 1)
					{
						...
						max_x = -1;
						max_y = -1;
						min_x = SHTPS_GET_PANEL_SIZE_X(ts);
						min_y = SHTPS_GET_PANEL_SIZE_Y(ts);
						...
						if( (max_x >= 0) && (max_y >= 0) &&
							(min_x < SHTPS_GET_PANEL_SIZE_X(ts)) && (min_y < SHTPS_GET_PANEL_SIZE_Y(ts)) ){
...
/* -------------------------------------------------------------------------- */
#if defined(SHTPS_DEF_GLOVE_DETECT_FAIL_RESOLVE_ENABLE)
static int shtps_glove_tu_timer_start(struct shtps_rmi_spi *ts, unsigned long delay_ms)
{
	SHTPS_GLOVE_DETECT_FAIL_RESOLVE_PRINT("tu timer start (%ldms)\n", delay_ms);

	cancel_delayed_work(&ts->filter_correct_p->glove_touch_up_delayed_work);
	schedule_delayed_work(&ts->filter_correct_p->glove_touch_up_delayed_work, msecs_to_jiffies(delay_ms));

	return 0;
}

static int shtps_glove_tu_timer_stop(struct shtps_rmi_spi *ts)
{
	SHTPS_GLOVE_DETECT_FAIL_RESOLVE_PRINT("tu timer stop\n");

	ts->filter_correct_p->glove_touch_up_delayed_finger = 0x00;
	cancel_delayed_work(&ts->filter_correct_p->glove_touch_up_delayed_work);

	return 0;
}

static void shtps_glove_touch_up_delayed_work_function(struct work_struct *work)
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct shtps_filter_correct_info *fc = container_of(dw, struct shtps_filter_correct_info, glove_touch_up_delayed_work);
	struct shtps_rmi_spi *ts = fc->ts_p;
	u8 	fingerMax = shtps_get_fingermax(ts);
	int	i;
	int isEvent = 0;

	SHTPS_GLOVE_DETECT_FAIL_RESOLVE_PRINT("delay tu func start\n");

	shtps_mutex_lock_ctrl();

	for(i = 0; i < fingerMax; i++){
		if( (ts->filter_correct_p->glove_touch_up_delayed_finger & (1 << i)) != 0 ){
			if(ts->report_info.fingers[i].state == SHTPS_TOUCH_STATE_GLOVE){
				isEvent = 1;
	...
	SHTPS_GLOVE_DETECT_FAIL_RESOLVE_PRINT("delay tu func end\n");

	return;
}

void shtps_filter_glove_tu_delay_check(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
	int fingerMax = shtps_get_fingermax(ts);
	int i;
	int diff_x, diff_y;

	if(SHTPS_PRM_GLOVE_DETECT_FAIL_RESOLVE_ENABLE == 0){
		return;
	}
	...
	for(i = 0;i < fingerMax;i++){
		if(ts->fw_report_info.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH){
			if((ts->filter_correct_p->glove_touch_up_delayed_finger & (1 << i)) == 0){
				if(ts->report_info.fingers[i].state == SHTPS_TOUCH_STATE_GLOVE){
					if(ts->filter_correct_p->glove_touch_up_delay_check_state[i] != 0){
						info->fingers[i].state = SHTPS_TOUCH_STATE_GLOVE;
						info->fingers[i].x  = ts->report_info.fingers[i].x;
						info->fingers[i].y  = ts->report_info.fingers[i].y;
						info->fingers[i].wx = ts->report_info.fingers[i].wx;
						info->fingers[i].wy = ts->report_info.fingers[i].wy;
						...
						ts->filter_correct_p->glove_touch_up_delayed_finger |= (1 << i);
						SHTPS_GLOVE_DETECT_FAIL_RESOLVE_PRINT("[%d]delay tu event(x=%d,y=%d,wx=%d,wy=%d,z=%d)\n",
																i, info->fingers[i].x, info->fingers[i].y,
																info->fingers[i].wx, info->fingers[i].wy, info->fingers[i].z);
						shtps_glove_tu_timer_start(ts, SHTPS_PRM_GLOVE_TU_DELAY_TIME_MS);
					}
				}
			}else{
				info->fingers[i].state = SHTPS_TOUCH_STATE_GLOVE;
				...
				SHTPS_GLOVE_DETECT_FAIL_RESOLVE_PRINT("[%d]delay tu event hold\n", i);
			}

			if(ts->filter_correct_p->glove_touch_up_delay_check_state[i] != 0){
				ts->filter_correct_p->glove_touch_up_delay_check_state[i] = 0;
				SHTPS_GLOVE_DETECT_FAIL_RESOLVE_PRINT("[%d]delay tu check end by tu\n", i);
			}
		}else{
			if(ts->fw_report_info_store.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH){
				if(ts->fw_report_info.fingers[i].state == SHTPS_TOUCH_STATE_GLOVE){
					ts->filter_correct_p->glove_detect_fail_resolve_td_x[i] = ts->fw_report_info.fingers[i].x;
					ts->filter_correct_p->glove_detect_fail_resolve_td_y[i] = ts->fw_report_info.fingers[i].y;
					ts->filter_correct_p->glove_touch_up_delay_check_state[i] = 1;
					SHTPS_GLOVE_DETECT_FAIL_RESOLVE_PRINT("[%d]delay tu check start\n", i);
				}else{
					ts->filter_correct_p->glove_touch_up_delay_check_state[i] = 0;
				}
			}else{
				if(ts->filter_correct_p->glove_touch_up_delay_check_state[i] != 0){
					if(SHTPS_PRM_GLOVE_TU_DELAY_CHECK_MOVE_THRESH > 0){
						diff_x = abs(ts->fw_report_info.fingers[i].x - ts->filter_correct_p->glove_detect_fail_resolve_td_x[i]);
						diff_y = abs(ts->fw_report_info.fingers[i].y - ts->filter_correct_p->glove_detect_fail_resolve_td_y[i]);

						if( (diff_x > SHTPS_PRM_GLOVE_TU_DELAY_CHECK_MOVE_THRESH) ||
							(diff_y > SHTPS_PRM_GLOVE_TU_DELAY_CHECK_MOVE_THRESH) )
						{
							ts->filter_correct_p->glove_touch_up_delay_check_state[i] = 0;
							SHTPS_GLOVE_DETECT_FAIL_RESOLVE_PRINT("[%d]delay tu check end by move thresh\n", i);
						}
					}
				}
			}
			...
			if((ts->filter_correct_p->glove_touch_up_delayed_finger & (1 << i)) != 0){
				ts->filter_correct_p->glove_touch_up_delayed_finger &= ~(1 << i);
				SHTPS_GLOVE_DETECT_FAIL_RESOLVE_PRINT("[%d]delay tu event cancel\n", i);
			}
		}
	}
}
...
void shtps_filter_correct_sleep(struct shtps_rmi_spi *ts)
{
	...
	#if defined(SHTPS_DEF_GLOVE_DETECT_FAIL_RESOLVE_ENABLE)
		shtps_glove_tu_timer_stop(ts);
		memset(ts->filter_correct_p->glove_touch_up_delay_check_state, 0, sizeof(ts->filter_correct_p->glove_touch_up_delay_check_state));
	#endif /* SHTPS_DEF_GLOVE_DETECT_FAIL_RESOLVE_ENABLE */
}

/* -------------------------------------------------------------------------- */
void shtps_filter_correct_forcetouchup(struct shtps_rmi_spi *ts)
{
	#if defined(SHTPS_DEF_GLOVE_DETECT_FAIL_RESOLVE_ENABLE)
		shtps_glove_tu_timer_stop(ts);
		memset(ts->filter_correct_p->glove_touch_up_delay_check_state, 0, sizeof(ts->filter_correct_p->glove_touch_up_delay_check_state));
...
void shtps_filter_correct_init(struct shtps_rmi_spi *ts)
{
	...
	#if defined(SHTPS_DEF_GLOVE_DETECT_FAIL_RESOLVE_ENABLE)
		INIT_DELAYED_WORK(&ts->filter_correct_p->glove_touch_up_delayed_work, shtps_glove_touch_up_delayed_work_function);
	#endif /* SHTPS_DEF_GLOVE_DETECT_FAIL_RESOLVE_ENABLE */
}
