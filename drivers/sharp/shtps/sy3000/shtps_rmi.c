...
static void shtps_facetouch_init(struct shtps_rmi_spi *ts)
{
	...
	ts->facetouch.qos_cpu_latency.type = PM_QOS_REQ_AFFINE_CORES;
	ts->facetouch.qos_cpu_latency.cpus_affine.bits[0] = 0x0f;
...
#if defined(SHTPS_CTRL_FW_REPORT_RATE)
int shtps_set_fw_report_rate_init(struct shtps_rmi_spi *ts)
{
...
static void shtps_lpwg_wakelock_init(struct shtps_rmi_spi *ts)
{
	...
	ts->lpwg.pm_qos_lock_idle.type = PM_QOS_REQ_AFFINE_CORES;
	ts->lpwg.pm_qos_lock_idle.cpus_affine.bits[0] = 0x0f;
...
static void shtps_set_touch_info(struct shtps_rmi_spi *ts, u8 *buf, struct shtps_touch_info *info)
{
	...
	#if defined(SHTPS_FINGER_KEY_EXCLUDE_ENABLE)
	{
		...
		if(is_finger_disable_check != 0){
			for(i = 0; i < fingerMax; i++){
				if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER){
					if( (info->fingers[i].y > (SHTPS_GET_PANEL_SIZE_Y(ts) - 1 - SHTPS_KEY_DISABLE_EFFECT_AREA)) ){
		...
		for(i = 0; i < fingerMax; i++){
			...
			{
				if( (ts->report_info.fingers[i].y > (SHTPS_GET_PANEL_SIZE_Y(ts) - 1 - SHTPS_KEY_DISABLE_EFFECT_AREA)) ){
...
//	SHTPS_LOG_FUNC_CALL();
	...
	#if defined(SHTPS_PEN_SCREEN_EDGE_DISABLE)
	{
		...
		if(pen_screen_edge_check != 0){
			...
			}else if(lcd_x > (CONFIG_SHTPS_SY3000_LCD_SIZE_X - SHTPS_PEN_EDGE_DISABLE_RANGE_X)){
				SHTPS_LOG_DBG_PRINT("[PEN_SCREEN_EDGE_DISABLE][%d] x = %d -> %d\n", pen, lcd_x, CONFIG_SHTPS_SY3000_LCD_SIZE_X - SHTPS_PEN_EDGE_DISABLE_RANGE_X);
				lcd_x = CONFIG_SHTPS_SY3000_LCD_SIZE_X - SHTPS_PEN_EDGE_DISABLE_RANGE_X;
			}
			...
			}else if(lcd_y > (CONFIG_SHTPS_SY3000_LCD_SIZE_Y - SHTPS_PEN_EDGE_DISABLE_RANGE_Y)){
				SHTPS_LOG_DBG_PRINT("[PEN_SCREEN_EDGE_DISABLE][%d] y = %d -> %d\n", pen, lcd_y, CONFIG_SHTPS_SY3000_LCD_SIZE_Y - SHTPS_PEN_EDGE_DISABLE_RANGE_Y);
				lcd_y = CONFIG_SHTPS_SY3000_LCD_SIZE_Y - SHTPS_PEN_EDGE_DISABLE_RANGE_Y;
			}
		}
	}
...
static int shtps_init_internal_variables(struct shtps_rmi_spi *ts)
{
	...
	shtps_performance_check_init();
		...
		ts->deter_suspend_spi.pm_qos_lock_idle.type = PM_QOS_REQ_AFFINE_CORES;
		ts->deter_suspend_spi.pm_qos_lock_idle.cpus_affine.bits[0] = 0x0f;
		...
		ts->pm_qos_lock_idle_proximity.type = PM_QOS_REQ_AFFINE_CORES;
		ts->pm_qos_lock_idle_proximity.cpus_affine.bits[0] = 0x0f;
						...
						int gpio_irq)
{
	...
	gShtps_rmi_spi = ts;
		...
		ts->tpin		= of_get_named_gpio(ctrl_dev_p->of_node, "shtps_rmi,tpin", 0);
	#else
