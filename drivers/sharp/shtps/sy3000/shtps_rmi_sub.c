 ...
 */
	...
	static void shtps_perflock_cluster0_enable(struct shtps_rmi_spi *ts);
	static void shtps_perflock_cluster0_disable(struct shtps_rmi_spi *ts);
	static void shtps_perflock_cluster0_enable_check(struct shtps_rmi_spi *ts, u8 event, struct shtps_touch_info *info);
	static int shtps_perflock_cluster0_disable_timer_start(struct shtps_rmi_spi *ts, unsigned long delay_ms);
	static void shtps_perflock_cluster0_disable_delayed_work_function(struct work_struct *work);

	static void shtps_perflock_cluster1_enable(struct shtps_rmi_spi *ts);
	static void shtps_perflock_cluster1_disable(struct shtps_rmi_spi *ts);
	static void shtps_perflock_cluster1_enable_check(struct shtps_rmi_spi *ts, u8 event, struct shtps_touch_info *info);
	static int shtps_perflock_cluster1_disable_timer_start(struct shtps_rmi_spi *ts, unsigned long delay_ms);
...
static void shtps_perflock_cluster0_enable(struct shtps_rmi_spi *ts)
{
	...
	shtps_mutex_lock_cpu_clock_ctrl();
		...
		if(SHTPS_CTRL_CPU_CLOCK_FOR_LOW_BRIGHTNESS_ENABLE == 0){
			if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_LCD_BRIGHT) != 0 ){
				SHTPS_LOG_DBG_PRINT("perf_lock <cluster0> not start because lcd bright is low.\n");
				req = 0;
			}
		}
		...
		if(SHTPS_CTRL_CPU_CLOCK_FOR_ECO_ENABLE == 0){
			if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_ECO) != 0 ){
				SHTPS_LOG_DBG_PRINT("perf_lock <cluster0> not start because eco mode is on.\n");
				req = 0;
			}
		}
			...
			if(enable_cpu_core_num < SHTPS_PERF_LOCK_CLUSTER0_ENABLE_CORE_NUM){
				if (cpu <= 1) {
						...
						if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_LCD_BRIGHT) == 0 ){
							...
							SHTPS_LOG_DBG_PRINT("perf_lock <cluster0> set [cpu=%d, freq=%d]\n",
	...
	if(SHTPS_PERF_LOCK_QOS_LATENCY_ENABLE != 0){
		if(ts->cpu_clock_ctrl_p->qos_cpu_cluster0_latency_state == 0){
			...
			SHTPS_LOG_DBG_PRINT("<cluster0> prohibit Standalone Powercollapse start\n");
		}
	}
...
static void shtps_perflock_cluster0_disable(struct shtps_rmi_spi *ts)
{
	...
	if(ts->cpu_clock_ctrl_p->qos_cpu_cluster0_latency_state == 1){
		...
		SHTPS_LOG_DBG_PRINT("<cluster0> prohibit Standalone Powercollapse end\n");
	}
...
static void shtps_perflock_cluster1_enable(struct shtps_rmi_spi *ts)
{
	...
	shtps_mutex_lock_cpu_clock_ctrl();
		...
		if(SHTPS_CTRL_CPU_CLOCK_FOR_LOW_BRIGHTNESS_ENABLE == 0){
			if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_LCD_BRIGHT) != 0 ){
				SHTPS_LOG_DBG_PRINT("perf_lock <cluster1> not start because lcd bright is low.\n");
				req = 0;
			}
		}
	#endif /* SHTPS_DEF_CTRL_CPU_CLOCK_LINKED_LCD_BRIGHT_ENABLE */
		...
		if(SHTPS_CTRL_CPU_CLOCK_FOR_ECO_ENABLE == 0){
			if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_ECO) != 0 ){
				SHTPS_LOG_DBG_PRINT("perf_lock <cluster1> not start because eco mode is on.\n");
				req = 0;
			}
		}
	#endif /* SHTPS_DEF_CTRL_CPU_CLOCK_LINKED_ECO_ENABLE */
			...
			if(enable_cpu_core_num < SHTPS_PERF_LOCK_CLUSTER1_ENABLE_CORE_NUM){
				if (cpu >= 2) {
						...
						if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_LCD_BRIGHT) == 0 ){
							...
							SHTPS_LOG_DBG_PRINT("perf_lock <cluster1> set [cpu=%d, freq=%d]\n",
	...
	if(SHTPS_PERF_LOCK_QOS_LATENCY_ENABLE != 0){
		if(ts->cpu_clock_ctrl_p->qos_cpu_cluster1_latency_state == 0){
			...
			SHTPS_LOG_DBG_PRINT("<cluster1> prohibit Standalone Powercollapse start\n");
		}
	}
...
static void shtps_perflock_cluster1_disable(struct shtps_rmi_spi *ts)
{
	...
	if(ts->cpu_clock_ctrl_p->qos_cpu_cluster1_latency_state == 1){
		...
		SHTPS_LOG_DBG_PRINT("<cluster1> prohibit Standalone Powercollapse end\n");
	}
...
static void shtps_perflock_cluster0_disable_delayed_work_function(struct work_struct *work)
{
	...
	SHTPS_LOG_DBG_PRINT("shtps_perflock_disable_timer end by Timer\n");
...
static void shtps_perflock_cluster0_enable_check(struct shtps_rmi_spi *ts, u8 event, struct shtps_touch_info *info)
{
	...
	if(SHTPS_PERF_LOCK_ENABLE_ALLEVENTS == 1
		...
		if( shtps_fwctl_get_dev_state(ts) == SHTPS_DEV_STATE_ACTIVE
			...
			shtps_perflock_cluster0_disable_timer_start(ts, perf_lock_enable_time /* ts->perf_lock_enable_time_ms */);
			SHTPS_LOG_DBG_PRINT("shtps_perflock_disable_timer start (%dms) by %s\n",
		...
		if(event == SHTPS_EVENT_TD){
			...
			if( shtps_fwctl_get_dev_state(ts) == SHTPS_DEV_STATE_ACTIVE
				...
				shtps_perflock_cluster0_enable(ts);
				shtps_perflock_cluster0_disable_timer_start(ts, SHTPS_PERF_LOCK_CLUSTER0_ENABLE_TIME_MS /* ts->perf_lock_enable_time_ms */);
				SHTPS_LOG_DBG_PRINT("shtps_perflock_disable_timer start by TouchDown\n");
			}
			...
			if(ts->cpu_clock_ctrl_p->report_event == SHTPS_EVENT_TD){
				...
				if( shtps_fwctl_get_dev_state(ts) == SHTPS_DEV_STATE_ACTIVE
					...
					shtps_perflock_cluster0_enable(ts);
					shtps_perflock_cluster0_disable_timer_start(ts, SHTPS_PERF_LOCK_CLUSTER0_ENABLE_TIME_MS /* ts->perf_lock_enable_time_ms */);
					SHTPS_LOG_DBG_PRINT("shtps_perflock_disable_timer start by Drag\n");
				}
			}
		}
...
static void shtps_perflock_cluster1_enable_check(struct shtps_rmi_spi *ts, u8 event, struct shtps_touch_info *info)
{
	...
	if(SHTPS_PERF_LOCK_ENABLE_ALLEVENTS == 1
		...
		if( shtps_fwctl_get_dev_state(ts) == SHTPS_DEV_STATE_ACTIVE
			...
			shtps_perflock_cluster1_disable_timer_start(ts, perf_lock_enable_time /* ts->perf_lock_enable_time_ms */);
			SHTPS_LOG_DBG_PRINT("shtps_perflock_disable_timer start (%dms) by %s\n",
								perf_lock_enable_time,
								( (event == SHTPS_EVENT_TD) ? "TouchDown" :
								  (event == SHTPS_EVENT_DRAG) ? "Drag" :
		...
		if(event == SHTPS_EVENT_TD){
			...
			if( shtps_fwctl_get_dev_state(ts) == SHTPS_DEV_STATE_ACTIVE
				...
				shtps_perflock_cluster1_enable(ts);
				shtps_perflock_cluster1_disable_timer_start(ts, SHTPS_PERF_LOCK_CLUSTER1_ENABLE_TIME_MS /* ts->perf_lock_enable_time_ms */);
				SHTPS_LOG_DBG_PRINT("shtps_perflock_disable_timer start by TouchDown\n");
			}
...
static void shtps_func_cover_on(struct shtps_rmi_spi *ts)
{
	shtps_cover_set_state(ts, 1);
}

static void shtps_func_cover_off(struct shtps_rmi_spi *ts)
{
...
static void shtps_func_boot_fw_update(struct shtps_rmi_spi *ts)
{
...
static void shtps_func_boot_fw_crc_check(struct shtps_rmi_spi *ts)
{
	u8 buf;
	const unsigned char* fw_data = NULL;
	u8 update = 0;
...
static void shtps_func_wakelock_init(struct shtps_rmi_spi *ts)
{
...
static void shtps_func_wakelock_deinit(struct shtps_rmi_spi *ts)
{
	wake_lock_destroy(&ts->work_wake_lock);
	pm_qos_remove_request(&ts->work_pm_qos_lock_idle);
}

static void shtps_func_wakelock(struct shtps_rmi_spi *ts, int on)
{
	SHTPS_LOG_FUNC_CALL();
	if(on){
		...
		switch(ts->cur_msg_p->event){
			...
			case SHTPS_FUNC_REQ_EVEMT_LCD_OFF:
				...
				case SHTPS_FUNC_REQ_BOOT_FW_UPDATE:
					...
					ts->cur_msg_p->status = 0;
					break;
				case SHTPS_FUNC_REQ_BOOT_FW_CRC_CHECK:
					shtps_func_boot_fw_crc_check(ts);
					ts->cur_msg_p->status = 0;
					break;
			#endif /* SHTPS_BOOT_FWUPDATE_ENABLE */
...
#if defined(SHTPS_DEF_RECORD_LOG_FILE_ENABLE)
void shtps_record_log_file_init(struct shtps_rmi_spi *ts)
{
...
void shtps_record_log_file_deinit(struct shtps_rmi_spi *ts)
{
	if(ts->record_log_file_info_p != NULL){
