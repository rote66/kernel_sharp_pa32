...
static void shtps_filter_change_position(struct shtps_offset_info *offset_p, int area, int *x, int *y)
{
	...
	if(area == 0x00){
		...
		base_xq = CONFIG_SHTPS_SY3000_LCD_SIZE_X;
		base_yp = 0;
		base_yq = offset_p->base[2];
	}else if(area == 0x03){
		xq = offset_p->diff[0];
		...
		base_yq = CONFIG_SHTPS_SY3000_LCD_SIZE_Y;
	}else if(area == 0x0A){
...
void shtps_filter_main(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
		...
		shtps_filter_temperature_ghost_reject_check(ts, info);
		...
		shtps_filter_glove_tu_delay_check(ts, info);
...
{
	#if defined(SHTPS_TEMPERATURE_GHOST_REJECT_CHECK_ENABLE)
		shtps_filter_temperature_ghost_reject_init(ts);
	#endif /* SHTPS_TEMPERATURE_GHOST_REJECT_CHECK_ENABLE */
...
{
	#if defined(SHTPS_TEMPERATURE_GHOST_REJECT_CHECK_ENABLE)
		shtps_filter_temperature_ghost_reject_deinit(ts);
	#endif /* SHTPS_TEMPERATURE_GHOST_REJECT_CHECK_ENABLE */
...
{
	#if defined(SHTPS_TEMPERATURE_GHOST_REJECT_CHECK_ENABLE)
		shtps_filter_temperature_ghost_reject_sleep(ts);
	#endif /* SHTPS_TEMPERATURE_GHOST_REJECT_CHECK_ENABLE */
...
{
	#if defined(SHTPS_TEMPERATURE_GHOST_REJECT_CHECK_ENABLE)
		shtps_filter_temperature_ghost_reject_forcetouchup(ts);
	#endif /* SHTPS_TEMPERATURE_GHOST_REJECT_CHECK_ENABLE */
	...
	#if defined(SHTPS_DEF_GLOVE_DETECT_FAIL_RESOLVE_ENABLE)
		shtps_filter_correct_forcetouchup(ts);
	#endif /* SHTPS_DEF_GLOVE_DETECT_FAIL_RESOLVE_ENABLE */
}
/* -------------------------------------------------------------------------- */
