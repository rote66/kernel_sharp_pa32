...
static int shtps_fwctl_s3400_initparam_set_custom_report_rate(struct shtps_fwctl_info *fc_p);
...
/* -------------------------------------------------------------------------- */
static char* shtps_fwctl_s3400_strstr(u8 *data_p, int data_length, u8 *search_p, int search_length)
{
	int i, j;
...
static int shtps_fwctl_s3400_make_serial_number_data(u8 *data_p, int data_length, u8 *buf_p)
{
	struct read_info_s {
		char	*search_str;
		int		read_length;
	...
	for(i = 0; i < (sizeof(read_info) / sizeof(struct read_info_s)); i++){
		...
		if(read_info[i].search_str != NULL){
			str_p = shtps_fwctl_s3400_strstr(data_p, data_length, read_info[i].search_str, strlen(read_info[i].search_str));
			if(str_p == NULL){
...
static int shtps_fwctl_s3400_get_serial_number(struct shtps_fwctl_info *fc_p, u8 *buf_p)
{
	...
	loopcnt = ((SHTPS_SERIAL_NUMBER_ALL_SIZE - 1) / blockSize) + 1;
	for(i = 0; i < loopcnt; i++){
		blockNumber = i;

		// write F34_BlockNumber and ConfigurationAreaSelect(0x01)
		blockNum[0] = blockNumber & 0xff;
		blockNum[1] = (blockNumber >> 8) & 0x1f;
		blockNum[1] |= (0x01 << 5);
		rc = M_WRITE_PACKET_FUNC(fc_p, fc_p->map_p->fn34.dataBase, blockNum, 2);
		if(rc){
			rc = -2;
			goto err_exit;
		}

		// write FlashCommand : Read Configuration Block(0x05)
		rc = M_READ_FUNC(fc_p, fc_p->map_p->fn34.dataBase + 2, &buf, 1);
		if(rc){
			rc = -3;
			goto err_exit;
		...
		rc = M_WRITE_FUNC(fc_p, fc_p->map_p->fn34.dataBase + 2, (buf & 0xC0) | (0x05 & 0x3F));
		if(rc){
			rc = -4;
			goto err_exit;
		}
		...
		retrycnt = 500;
		while(retrycnt){
			rc = M_READ_FUNC(fc_p, fc_p->map_p->fn34.dataBase + 3, &buf, 1);
			if(rc){
				rc = -5;
				goto err_exit;
			}
		...
		// read block data
		if(i == (loopcnt - 1)){
			readsize = SHTPS_SERIAL_NUMBER_ALL_SIZE % blockSize;
		}
		else{
			readsize = blockSize;
		}
		rc = M_READ_PACKET_FUNC(fc_p, fc_p->map_p->fn34.dataBase + 1, &serial_all[blockSize * i], readsize);
		if(rc){
			rc = -7;
			goto err_exit;
		}
	...
	// make serialnumber strings
	rc = shtps_fwctl_s3400_make_serial_number_data(serial_all, SHTPS_SERIAL_NUMBER_ALL_SIZE, serial_data);
	if(rc){
		rc = -8;
		goto err_exit;
	}
...
/* -------------------------------------------------------------------------- */
static int shtps_fwctl_s3400_cmd_tm_hybrid_adc(struct shtps_fwctl_info *fc_p, u8 tm_mode)
{
	int rc;
	SHTPS_LOG_FWCTL_FUNC_CALL();
...
#if defined( SHTPS_CTRL_FW_REPORT_RATE )
	SHTPS_LOG_FWCTL_FUNC_CALL();
	return shtps_fwctl_s3400_set_custom_report_rate(fc_p, fc_p->map_p->fn51_custom_report_rate_def);
#else
	return 0;
#endif	/* SHTPS_CTRL_FW_REPORT_RATE */
}
...
struct shtps_fwctl_functbl		shtps_fwctl_s3400_function_table = {
		...
		.initparam_set_custom_report_rate_f			= shtps_fwctl_s3400_initparam_set_custom_report_rate,
