...
#if defined(SHDISP_SYSFS_LED) || defined(SHDISP_SYSFS_DIM)
#include "shdisp_led.h"
#endif /* SHDISP_SYSFS_LED || SHDISP_SYSFS_DIM */
#include "./data/shdisp_typical_igc_data_default.h"
...
extern void mdss_shdisp_xlog(int no, bool onoff);
extern int  mdss_shdisp_mdp_cmd_wait4ppdone(void);
extern int mdss_shdisp_wait4lock_atomic_commit(void);
...
static void shdisp_led_set_dim(struct led_classdev *led_cdev, enum led_brightness value);
...
static struct shdisp_gc_lut *shdisp_get_gc_lut(void);
...
#ifdef SHDISP_SYSFS_DIM
static int shdisp_SQE_LED_set_dim(int value);
#endif /* SHDISP_SYSFS_DIM */
...
static int shdisp_do_recovery(void);
#endif  /* SHDISP_NOT_SUPPORT_DET */
...
#ifdef SHDISP_SYSFS_DIM
static struct led_classdev dim_led = {
        .name           = "dim",
        .brightness     = LED_OFF,
        .brightness_set = shdisp_led_set_dim,
        .max_brightness = 1,
};
#endif /* SHDISP_SYSFS_DIM */
...
/* ------------------------------------------------------------------------- */
/* shdisp_api_get_gc_lut                                                     */
/* ------------------------------------------------------------------------- */
struct shdisp_gc_lut *shdisp_api_get_gc_lut(void)
{
    return shdisp_get_gc_lut();
}
...
static void shdisp_boot_context_initialize(void)
{
    ...
    for (i = 0; i < SHDISP_GC_LUT_ENTRIES; i++) {
        if (i == 0) {
            shdisp_kerl_ctx.boot_ctx.gc_lut.r_data[i] = 0x0000;
            shdisp_kerl_ctx.boot_ctx.gc_lut.g_data[i] = 0x0000;
            shdisp_kerl_ctx.boot_ctx.gc_lut.b_data[i] = 0x0000;
        } else {
            shdisp_kerl_ctx.boot_ctx.gc_lut.r_data[i] = 0x0004;
            shdisp_kerl_ctx.boot_ctx.gc_lut.g_data[i] = 0x0004;
            shdisp_kerl_ctx.boot_ctx.gc_lut.b_data[i] = 0x0004;
        }
    }
...
static struct shdisp_gc_lut *shdisp_get_gc_lut(void)
{
    return &shdisp_kerl_ctx.boot_ctx.gc_lut;
}
...
static struct shdisp_igc_lut *shdisp_get_igc_lut(void)
{
    return (struct shdisp_igc_lut*)&igc_data;
}
...
#ifdef SHDISP_SYSFS_DIM
/* ------------------------------------------------------------------------- */
/* shdisp_SQE_LED_set_dim                                                    */
/* ------------------------------------------------------------------------- */
static int shdisp_SQE_LED_set_dim(int value)
{
    shdisp_bdic_API_LED_set_dim(value);
...
static int shdisp_SQE_do_recovery(void)
{
    ...
    shdisp_IO_API_usleep(WAIT_1FRAME_US);
...
static void shdisp_SQE_lcd_det_recovery(void)
{
    ...
    for (i = 0; i < retry_max; i++) {
        ...
        mdss_shdisp_xlog(SHDISP_XLOG_RECOVERING, true);
        shdisp_SQE_do_recovery();
        mdss_shdisp_xlog(SHDISP_XLOG_RECOVERING, false);
...
static void shdisp_lcd_det_recovery(void)
{
    ...
    SHDISP_TRACE("in");
    mdss_shdisp_xlog(SHDISP_XLOG_RECOVERY, true);
    ...
    ret = mdss_shdisp_wait4lock_atomic_commit();
    if(ret != SHDISP_RESULT_SUCCESS) {
        SHDISP_ERR("failed to wait atomic commit");
    }

    mdss_shdisp_lock_recovery();

#if defined(CONFIG_SHDISP_PANEL_HAYABUSA)
    mdss_shdisp_mdp_cmd_wait4ppdone();
#endif /* CONFIG_SHDISP_PANEL */
    ...
    mdss_shdisp_xlog(SHDISP_XLOG_RECOVERY, false);
    SHDISP_TRACE("out");
    return;
}
...
static void shdisp_led_set_dim(struct led_classdev *led_cdev, enum led_brightness value)
{
    ...
    if ((value != SHDISP_LED_SYSFS_DIM_OFF) && (value != SHDISP_LED_SYSFS_DIM_ON)) {
        SHDISP_DEBUG("out of dim value");
        return;
    }

    SHDISP_DEBUG("/sys/class/leds/dim/brightness is %d", value);
    shdisp_semaphore_start();

    shdisp_SQE_LED_set_dim(value);
...
static void shdisp_dbg_info_output(int mode)
{
    ...
    case SHDISP_DEBUG_INFO_TYPE_BOOT:
        ...
        printk("[SHDISP] boot_ctx.gc_lut.r_data                = ");
        for (i = 0; i < SHDISP_GC_LUT_ENTRIES; i++) {
            if (i % 16 == 0) {
                printk("\n    ");
            }
            printk("%02X,", shdisp_kerl_ctx.boot_ctx.gc_lut.r_data[i]);
        }
        printk("\n");
        printk("[SHDISP] boot_ctx.gc_lut.g_data                = ");
        for (i = 0; i < SHDISP_GC_LUT_ENTRIES; i++) {
            if (i % 16 == 0) {
                printk("\n    ");
            }
            printk("%02X,", shdisp_kerl_ctx.boot_ctx.gc_lut.g_data[i]);
        }
        printk("\n");
        printk("[SHDISP] boot_ctx.gc_lut.b_data                = ");
        for (i = 0; i < SHDISP_GC_LUT_ENTRIES; i++) {
            if (i % 16 == 0) {
                printk("\n    ");
            }
            printk("%02X,", shdisp_kerl_ctx.boot_ctx.gc_lut.b_data[i]);
        }
        printk("\n");
...
#ifdef SHDISP_SYSFS_DIM
            if (led_classdev_register(&pdev->dev, &dim_led)) {
                SHDISP_ERR("led_classdev_register(dim) failed");
            }
#endif /* SHDISP_SYSFS_DIM */
        } else {
