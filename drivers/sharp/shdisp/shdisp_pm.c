...
#define GET_DEV_STATE_STR(n)    (((n) == SHDISP_DEV_STATE_OFF)    ? "OFF"    : \
...
void shdisp_pm_API_init(struct shdisp_boot_context *shdisp_boot_ctx)
{
    ...
    shdisp_pm_ctx_recovery.bdic_status.power_status  = SHDISP_DEV_STATE_OFF;
...
#ifdef SHDISP_APPSBL
    shdisp_API_set_boot_context_psals(&shdisp_pm_ctx.psals_status);
#endif  /* SHDISP_APPSBL */
...
static int shdisp_pm_bdic_set_active(void)
{
    ...
    ret = shdisp_bdic_API_set_active();
