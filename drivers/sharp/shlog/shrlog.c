				    ...
				    int restart_mode, int in_panic)
{
	...
	if ((cmd != NULL) && !strncmp(cmd, "surfaceflinger", 14)) {
		...
		shrlog_set_restart_reason(0x77665594);
	}
#endif /* CONFIG_SHARP_HANDLE_SURFACE_FLINGER */
}
...
static int shrlog_set_apps_info(shrlog_fixed_apps_info *apps_info)
{
	...
	apps_info->module_init_offset = offsetof(struct module, module_init);
	apps_info->module_core_offset = offsetof(struct module, module_core);
	apps_info->module_init_size_offset = offsetof(struct module, init_size);
	apps_info->module_core_size_offset = offsetof(struct module, core_size);
