...
static ssize_t proximity_proc_file_write(struct file *file, const char __user *buffer, size_t count, loff_t *data);
#endif /* CONFIG_ANDROID_ENGINEERING */
...
#ifdef CONFIG_SHTERM
	shterm_k_set_info(SHTERM_INFO_PROXIMITY, 1);
#endif
...
#ifdef CONFIG_SHTERM
	shterm_k_set_info(SHTERM_INFO_PROXIMITY, 0);
#endif
...
/*  ------------------------------------------------------------------------- */
/*  proximity_proc_file_write                                                    */
/*  ------------------------------------------------------------------------- */
static ssize_t proximity_proc_file_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	ssize_t rv = -EIO;

	rv = proximity_proc_write(file, buffer, count, data);
...
#ifdef PROX_DRV_MSM8992
static struct platform_device_id prox_id_table[] = {
	{ PROX_DRIVER_NAME, 0 },
	{ }
};

static struct of_device_id prox_match_table[] = {
	{ .compatible = PROX_DRIVER_NAME, },
	{},
};

MODULE_DEVICE_TABLE(platform, prox_id_table);
static struct platform_driver gp2a_prox_driver = {
	.probe = PROX_Probe,
	.id_table = prox_id_table,
	.driver = {
...
static int __init PROX_Init(void)
{
	...
	proc_fops.write = proximity_proc_file_write;
	entry = proc_create("driver/PROXIMITY_CAL", 0666, NULL, &proc_fops);
