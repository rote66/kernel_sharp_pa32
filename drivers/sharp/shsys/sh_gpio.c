 ...
 * Register Address
 */
// LINUX/android/bootable/bootloader/lk/platform/msm8996/include/platform/iomap.h
...
#if 0	/* temprary comment for build error start */
...
#endif	/* temprary comment for build error end */
...
static int sh_gpio_ioctl_write(unsigned long arg)
{
    ...
    return rc;
#else	//  temprary comment for build error
    return 0;
#endif	/* temprary comment for build error end */
}
