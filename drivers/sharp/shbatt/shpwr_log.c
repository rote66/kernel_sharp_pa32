...
#define SHPWR_DBG_LOG_ONE_LINE_FORMAT(length, message) length = snprintf((char *)shpwr_dbg_log_buf + shpwr_dbg_log_buf_cur_index * SHPWR_DBG_LOG_MSG_MAX_LENGTH,\
                       SHPWR_DBG_LOG_MSG_MAX_LENGTH,\
                      "%04d/%02d/%02d(%s), %02d:%02d:%02d, UTC +%02dh : %s",\
...
void shpwr_add_dbg_log(char *fmt, ...)
{
    ...
    if (enable_durable_log) {
        ...
        if ((index >= SHPWR_DBG_LOG_LINE_SIZE) && is_shbatt_prs_launched()) {
...
int shpwr_dbg_log_add_file(void)
{
    ...
    if (shpwr_dbg_kernel_seek(fp, 0, SEEK_END)
        ...
        do {
            ...
            ret = kern_path(SHPWR_DBG_LOG_FILE, LOOKUP_PARENT, &path);
            if (ret) {
                SHPWR_ERROR("kern_path_parent error ret=%d\n", ret);
                break;
            }
            lock_rename(path.dentry, path.dentry);

            d_from = lookup_one_len(SHPWR_DBG_LOG_FNAME, path.dentry, strlen(SHPWR_DBG_LOG_FNAME));
            if (IS_ERR_OR_NULL(d_from)) {
                unlock_rename(path.dentry, path.dentry);
                path_put(&path);
                SHPWR_ERROR("d_from is invalid\n");
                break;
            }
            ...
            if (!d_from->d_inode) {
                ...
                unlock_rename(path.dentry, path.dentry);
                path_put(&path);
                SHPWR_ERROR("d_from->d_inode is null\n");
                break;
            }

            d_to = lookup_one_len(SHPWR_DBG_LOG_OLD_FNAME, path.dentry, strlen(SHPWR_DBG_LOG_OLD_FNAME));
            if (IS_ERR_OR_NULL(d_to)) {
                dput(d_from);
                unlock_rename(path.dentry, path.dentry);
                path_put(&path);
                SHPWR_ERROR("d_to is invalid\n");
                break;
            }

/* [SHPWR][#58664] Temp Modification START */
            //ret = vfs_rename(path.dentry->d_inode, d_from, path.dentry->d_inode, d_to);
            ret = vfs_rename(path.dentry->d_inode, d_from, path.dentry->d_inode, d_to, NULL, 0);
/* [SHPWR][#58664] Temp Modification END */
            unlock_rename(path.dentry, path.dentry);
            dput(d_from);
            dput(d_to);
            path_put(&path);
            if (ret) {
                SHPWR_ERROR("vfs_rename error ret=%d\n", ret);
                break;
            }
...
void shpwr_add_dump_reg(bool force_save_flg, char *fmt, ...)
{
    ...
    if (enable_dump_reg) {
        ...
        if ((force_save_flg) && is_shbatt_prs_launched()) {
...
int shpwr_dump_reg_add_file(void)
{
    ...
    if (shpwr_dbg_kernel_seek(fp, 0, SEEK_END)
        ...
        do {
            memset(&path, 0, sizeof(struct path));
            ret = kern_path(SHPWR_DUMP_REG_FILE, LOOKUP_PARENT, &path);
            if (ret) {
                SHPWR_ERROR("kern_path_parent error ret=%d\n", ret);
                break;
            }
            ...
            d_from = lookup_one_len(SHPWR_DUMP_REG_FNAME, path.dentry, strlen(SHPWR_DUMP_REG_FNAME));
            if (IS_ERR_OR_NULL(d_from)) {
                unlock_rename(path.dentry, path.dentry);
                path_put(&path);
                SHPWR_ERROR("d_from is invalid\n");
            ...
            d_to = lookup_one_len(SHPWR_DUMP_REG_OLD_FNAME, path.dentry, strlen(SHPWR_DUMP_REG_OLD_FNAME));
            if (IS_ERR_OR_NULL(d_to)) {
                dput(d_from);
                unlock_rename(path.dentry, path.dentry);
                path_put(&path);
...
/* [SHPWR][#58664] Temp Modification END */
    ...
    snprintf((char *)buf, SHPWR_DUMP_REG_MSG_MAX_LENGTH, "%04d/%02d/%02d(%s), %02d:%02d:%02d, UTC +00h : \n",
            ...
            tm1.tm_sec);
#else
    snprintf((char *)buf, SHPWR_DUMP_REG_MSG_MAX_LENGTH, "%04d/%02d/%02d(%s), %02d:%02d:%02d, UTC +%02dh : \n",
