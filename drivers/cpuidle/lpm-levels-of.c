...
uint32_t max_fpc_latency = 0;
int get_latency_value(int mode){
	uint32_t latency = 0;

	if(mode == MSM_PM_SLEEP_MODE_FASTPC)
		latency = max_fpc_latency;
...
#ifdef CONFIG_SHSYS_CUST_DEBUG
		if(!strcmp(l->name, "fpc")){
			if(max_fpc_latency < l->pwr.latency_us)
				max_fpc_latency = l->pwr.latency_us;
		}
#endif /* CONFIG_SHSYS_CUST_DEBUG */
